...
### Changed
- **One changelog** - `CHANGELOG.md` at the repository root, and nothing else. cbox.dk takes the changelog from the root file, so the `docs/changelog.md` copy bought nothing and is deleted, along with the lint gate that was supposed to keep the two in step: it compared them in one direction only (root must carry docs' newest release), so it could fail when the root fell behind and not when docs did. Docs did, sitting at 1.5.1 for two minor releases while the root reached 1.7.0, green the whole way

### Added
- **`cboxdk/telemetry-native` ships in every tier from slim up (opt-in, `PHP_TELEMETRY_NATIVE=true`)** - the native half of `cboxdk/laravel-telemetry`: a statistical CPU profiler on a per-thread CPU timer, native timing for PDO/Redis connects and `curl_exec`, bounded runtime counters, and a signal-safe crash recorder that re-raises so the process still dies with its core dump. laravel-telemetry can say a request burned 410 ms of PHP CPU; this says which call stacks burned it. Built from source at the pinned tag (it is not on PECL), PHP 8.3+ only - the 8.2 images skip it and say so. **Off by default because it is pre-1.0 and its crash recorder installs fatal-signal handlers, NOT because it costs anything**: upstream's FPM measurements put loading it, installing the hooks and arming the recorder inside the harness's own noise floor, and profiling every request at 1 ms between nothing and ~5%. Same read-only-safe mechanism as the otel gate - the ini lives outside the default scan dir and the entrypoints append it to `PHP_INI_SCAN_DIR`, so nothing is written at boot. `PHP_TELEMETRY_NATIVE_AUTO=true` additionally opens a unit at `RINIT` so profiles cover framework boot: right for FPM, deliberately a separate switch because `RINIT` fires once per *process* in queue workers and Octane. The build asserts both the version it produced and `timer_backend=posix-thread-cputime`, so a silently wall-clock profiler fails the image instead of shipping. The 8.6 beta lane attempts it too, from the same pinned source tag: `OK cbox_telemetry source:v0.1.0 posix-thread-cputime` on 8.6.0beta2 - it compiles and runs a real per-thread CPU timer there despite declaring `^8.3|^8.4|^8.5`
- **PHP 8.6 beta lane, all four tiers (experimental, issue #23)** - `php-base/Dockerfile.php86-beta` builds slim/standard/chromium/dev against `php:8.6-rc` with the pecl-free PIE path, weekly and on demand, pushing clearly-marked `8.6-rc-bookworm{-slim,-,-chromium,-dev}-beta` tags. Blocked extensions are ATTEMPTED tolerantly: failures land in a per-image report (surfaced in the CI job summary) instead of killing the build, so the report flips to OK the week upstream tags an 8.6 fix. Discoveries so far on 8.6.0beta2: the stable `pie` binary refuses to run on 8.6 (nightly required); `mbstring` must be installed explicitly; 8.6's gd links libwebpdemux/libwebpmux; **all 24 php-src extensions across the tiers compile** (incl. soap/xsl/ldap/sysv*); **SPX does NOT compile on 8.6 - v0.4.22 nor master, a genuine upstream gap**; released redis/apcu/msgpack/imagick fail in make as predicted, but **all six blockers build from their dev branches** (redis dev-develop at 6.3.0RC1, apcu dev-master, msgpack dev-master, imagick dev-master, xdebug 3.6.0alpha1, pcov's unmerged fix branch dev-issue-php86a2), so the beta images carry the full production extension set except vips and SPX - and the -DEV attempts retire themselves the week released tags land. Browser stack (chromium, chromedriver, baked Playwright) carries over unchanged; mongodb + opentelemetry via PIE, otel behind the same opt-in gate as production

### Fixed
- **`php-base` and `php-cli` dev tiers shipped without `git`** - `dev-base` listed git among its build deps and then ran `apt-get purge -y --auto-remove $PHPIZE_DEPS git`, so the one tier that exists for CI and local development lost it; `--auto-remove` took `git-man`, `liberror-perl` and `patch` (which cweagans/composer-patches needs) along with it. Verified against the published images rather than the source: `git --version` is `not found` in both `php-base:8.4-bookworm-dev` and `php-cli:8.4-bookworm-dev` (which extends php-base directly), while the standard tiers of both have it. `php-fpm` and `php-fpm-nginx` dev images are NOT affected - they build FROM the official php-fpm image with their own apt layer and only COPY extensions out of php-base. git is a runtime dependency installed in slim-base - the baked `safe.directory = *` exists solely to make it work on bind-mounted apps - so it is no longer named in the dev tier's install or purge at all; the SPX clone uses the git that is already there. Rebuilt and confirmed: the dev tier now drops nothing relative to the chromium tier it extends. New `tests/test-runtime-deps.sh` fails any stage that purges a package slim-base installs as a runtime dep (verified to fail on the old line), and the e2e base scenario asserts git/ssh/unzip/curl plus `safe.directory` on the built image
- **arm64 built OpenTelemetry from the Dockerfile default, not `versions.json`** - the shared build workflow has two near-identical jobs, and only the amd64 one read `extensions.opentelemetry` out of `versions.json` and passed it as a build arg. The arm64 job fell back to the `ARG OPENTELEMETRY_VERSION` baked into `php-base/Dockerfile`, so the moment the weekly updater bumped the pin, the two architectures behind the same tag could carry different otel versions until somebody also edited the Dockerfile default. Both jobs now read and pass it; the version steps and php-base build-arg blocks are byte-identical between them, and the remaining differences are genuinely arch-specific (artifact name, the amd64-only disk-space step)

## [1.7.0] - 2026-09-14

### Added
- **Pest v4 browser testing works out of the box on the chromium tier** - the released pest-plugin-browser cannot use a system Chromium (Playwright allows no executable override; watch pest-plugin-browser#243), so the image bakes Playwright's own Chromium at `PLAYWRIGHT_BROWSERS_PATH=/ms-playwright`, revision-refreshed to `playwright@latest` by the weekly rebuild: `composer require pestphp/pest-plugin-browser && npm i playwright && vendor/bin/pest` with zero browser downloads. Launch smoke validated (root, no sandbox flags - Playwright defaults chromiumSandbox=false). Both plugin majors verified individually end-to-end against the same baked revision: Pest 4 (4.7.8 + plugin 4.3.1) and Pest 5 (5.1.4 + plugin 5.0.1), real composer projects, real browser tests, zero downloads
...
- **GitHub Actions guide + standing verification** - `docs/guides/github-actions.md`: run CI inside the exact image you deploy (job containers with checkout working out of the box thanks to the baked `safe.directory`), databases as sibling services, browser tests with zero Playwright downloads, and php-fpm-nginx as a health-gated service container. Every claim is continuously proven by `.github/workflows/verify-actions-usage.yml` in a real runner, re-run weekly right after the image rebuilds - verified green before this release
...
- **E2E: git-integration plumbing scenario** - asserts git/ssh/ssh-keyscan presence, `git status` as www-data in a root-owned repo (safe.directory), and commit+push as the runtime user to a bare remote. Verified failing on pre-fix images and green on this one
...
### Changed
- **The OpenTelemetry extension is opt-in (`PHP_OPENTELEMETRY=true`)** - loading it enables the Zend observer API, which taxes every PHP function call even with no OTel SDK installed: measured on dedicated EPYC hardware at **-18.5% throughput on a Laravel app** (0% on tight-loop code; the cost scales with function-call density). This was the single largest gap against serversideup on the cloud benchmark - with the extension unloaded the same image went from -11% to **+5% on the Laravel fixture**. The ini now lives in `conf.d-otel/`; the entrypoints append it to `PHP_INI_SCAN_DIR` when enabled, so the gate works on read-only filesystems
